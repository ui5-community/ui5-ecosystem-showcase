...
export class TrippinService extends ODataService {
	...
	people(id) {
		...
		return typeof id === "undefined" || id === null ? new PersonCollectionService(client, path, fieldName) : new PersonService(client, path, new QPersonId(fieldName).buildUrl(id));
	}
	airlines(id) {
		const fieldName = "Airlines";
		const { client, path } = this.__base;
		return typeof id === "undefined" || id === null ? new AirlineCollectionService(client, path, fieldName) : new AirlineService(client, path, new QAirlineId(fieldName).buildUrl(id));
	}
	airports(id) {
		const fieldName = "Airports";
		const { client, path } = this.__base;
		return typeof id === "undefined" || id === null ? new AirportCollectionService(client, path, fieldName) : new AirportService(client, path, new QAirportId(fieldName).buildUrl(id));
	}
	me() {
		if (!this._me) {
			const { client, path } = this.__base;
...
export class PersonService extends EntityTypeServiceV4 {
	...
	trips(id) {
		...
		const { client, path } = this.__base;
		return typeof id === "undefined" || id === null ? new TripCollectionService(client, path, fieldName) : new TripService(client, path, new QTripId(fieldName).buildUrl(id));
	}
...
export class TripService extends EntityTypeServiceV4 {
	...
	planItems(id) {
		...
		const { client, path } = this.__base;
		return typeof id === "undefined" || id === null ? new PlanItemCollectionService(client, path, fieldName) : new PlanItemService(client, path, new QPlanItemId(fieldName).buildUrl(id));
	}
