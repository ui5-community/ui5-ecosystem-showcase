	...
	<div id="trippin" class="ui5-content-density-compact">
		<div class="u-bar">
			...
			<div class="u-bar-singleLineContent space-x-0 md:space-x-2">
				<ui5-input class="w-full md:w-32" ref="searchField" show-clear-icon @input="handleSearch" @change="handleSearch" placeholder="Search">
					<ui5-icon id="searchIcon" slot="icon" name="search" />
				</ui5-input>
			</div>
		</div>
		<ui5-list class="full-width" id="personList" ref="list" mode="SingleSelect" :growing="this.growing" :busy="this.busy" @load-more="handleLoadMore">
			<ui5-li v-for="person in persons" :key="person.userName" :icon="person.gender === 'Male' ? 'male' : 'female'" :additional-text="person.emails[0] ? person.emails[0] : '<unknown>'">
	...
	methods: {
		...
		fetchData: async function (more) {
			...
			if (this.query.length > 0) {
				...
				filter = qPerson.firstName.toUpper().contains(upperQuery).or(qPerson.lastName.toUpper().contains(upperQuery));
			}

			// fetch data using api
			const response = await trippinService.people().query((builder) =>
				builder
					.count()
