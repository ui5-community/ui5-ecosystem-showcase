...
module.exports = async ({workspace, dependencies, options}) => {
    ...
    const fileExtensions = Array.isArray(config.fileExtensions) ?
        config.fileExtensions : ([config.fileExtensions || "xml"]);
    const xmlResources = await workspace.byGlob(`**/*.+(${ fileExtensions.join("|")})`);

    await Promise.all(xmlResources.map(async resource => {
        ...
        let xml = minify(await resource.getString(), options.minifyOptions);
        ...
        if (options.minifyOptions.collapseWhitespaceInAttributeValues) {
