...
module.exports = function (log, projectInfo) {
	...
	const that = {
		 ...
		 * @param {object} [config.inject] the inject configuration for @rollup/plugin-inject
		 * @param {boolean|string} [config.addToNamespace] Adds modules into the sub-namespace thirdparty of the Component
		 * @param {boolean} [config.useRelativeModulePaths] Flag whether the modules should use relative paths instead of absolute paths (for applications)
		 * @param {object} [options] additional options
		 * @param {string} [options.cwd] current working directory
		 * @param {string[]} [options.depPaths] paths of the dependencies (in addition for cwd)
				...
				if (modules.length > 0) {
					...
					if (skipCache || !BundleInfoCache.get(cacheKey, bundleCacheOptions)) {
						...
						const replaceParam = function (code, search, replacement) {
							...
							return code.replace(regex, `$1${replacement}$2$3`);
						};

						// fix the imports for shifted modules
						for (const module of bundleInfo.getEntries()) {
							// the imports must not be relative to the /resources path as this would break the module resolution
							// e.g. when you call ../ which points to resources, it will loose the resource roots mapping:
							// ---
							// Example:
							...
							// HINT: useRelativeModulePaths => UI5 testsuite scenario using control test pages
							//       as they all do not define the testsuite namespace as root namespace
							//       for them we need to use the relative module paths for proper loading
							if (!useRelativeModulePaths && (isMiddleware || !addToNamespace) && projectType === "application" && projectNamespace) {
								...
								moduleBasePath = `${path.posix.relative(path.dirname(module.name), "") || "."}/`;
							}
							...
							} else if (!addToNamespace && module.generated) {
