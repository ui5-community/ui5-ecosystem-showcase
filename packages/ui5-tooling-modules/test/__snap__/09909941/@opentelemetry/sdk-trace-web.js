sap.ui.define(['exports', '../trace-api'], (function (exports, traceApi) { 'use strict';
    ...
    function validateSchemaUrl(schemaUrl) {
        ...
        traceApi.diag.warn('Schema URL must be string or undefined, got %s. Schema URL will be ignored.', schemaUrl);
        return undefined;
    }
    function mergeSchemaUrl(old, updating) {
        ...
        traceApi.diag.warn('Schema URL merge conflict: old resource has "%s", updating resource has "%s". Resulting resource will have undefined Schema URL.', oldSchemaUrl, updatingSchemaUrl);
        return undefined;
    }
    ...
    class SpanImpl {
        ...
        addLink(link) {
            ...
            if (linkCountLimit !== undefined && this.links.length >= linkCountLimit) {
                if (this._droppedLinksCount === 0) {
                    traceApi.diag.debug('Dropping extra links.');
                }
                this.links.shift();
                this._droppedLinksCount++;
            }
        ...
        setStatus(status) {
            if (this._isSpanEnded())
                return this;
            if (status.code === traceApi.SpanStatusCode.UNSET)
                return this;
            if (this.status.code === traceApi.SpanStatusCode.OK)
                return this;
            ...
            if (status.code === traceApi.SpanStatusCode.ERROR) {
        ...
        updateName(name) {
            ...
            if (this._droppedLinksCount > 0) {
                traceApi.diag.warn(`Dropped ${this._droppedLinksCount} links because linkCountLimit reached`);
            }
    ...
    function buildSamplerFromEnv() {
        ...
        switch (sampler) {
            ...
            default:
                traceApi.diag.error(`OTEL_TRACES_SAMPLER value "${sampler}" invalid, defaulting to "${TracesSamplerValues.ParentBasedAlwaysOn}".`);
                return new ParentBasedSampler({
                    root: new AlwaysOnSampler(),
                });
        }
