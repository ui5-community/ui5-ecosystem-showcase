sap.ui.define(['./webcomponents', './Keys'], (function (webcomponents, Keys) { 'use strict';
  ...
  const u$1=o=>{let i,a,r,d,n,s,p,g,e,l,y,m;const h=f$2({beforeStart:()=>{const t=o;i=getComputedStyle(t),a=parseFloat(i.paddingTop),r=parseFloat(i.paddingBottom),d=parseFloat(i.marginTop),n=parseFloat(i.marginBottom),s=parseFloat(i.height),p=t.style.overflow,g=t.style.paddingTop,e=t.style.paddingBottom,l=t.style.marginTop,y=t.style.marginBottom,m=t.style.height,t.style.overflow="hidden";},duration:v$1,element:o,advance:t=>{o.style.paddingTop=`${a-a*t}px`,o.style.paddingBottom=`${r-r*t}px`,o.style.marginTop=`${d-d*t}px`,o.style.marginBottom=`${n-n*t}px`,o.style.height=`${s-s*t}px`;}});return h.promise().then(t=>{t instanceof Error||(o.style.overflow=p,o.style.paddingTop=g,o.style.paddingBottom=e,o.style.marginTop=l,o.style.marginBottom=y,o.style.height=m,o.style.display="none");}),h};

  var s = ":host{justify-content:center;align-items:center;display:flex;color:var(--sapList_TextColor);background-color:var(--sapList_Background);font-family:var(--sapFontFamily);height:var(--sapElement_LineHeight);border-bottom:var(--sapList_BorderWidth) solid var(--sapList_BorderColor);opacity:.8;position:absolute;padding:0 1rem;top:-1000px;left:-1000px}";

  const e={key:"DRAG_DROP_MULTIPLE_TEXT",defaultText:"{0} items"};

  const l$2=2;const u=async e$1=>{const t=document.createElement("div"),n=await webcomponents.f("@ui5/webcomponents-base"),r=t.attachShadow({mode:"open"}),o=new CSSStyleSheet;return o.replaceSync(s),r.adoptedStyleSheets=[o],r.textContent=n.getText(e,e$1),t},m=async(e,t)=>{if(e<l$2){console.warn(`Cannot start multiple drag with count ${e}. Minimum is ${l$2}.`);return}if(!t.dataTransfer)return;const n=await u(e);document.body.appendChild(n),t.dataTransfer.setDragImage(n,0,0),requestAnimationFrame(()=>{n.remove();});};
  ...
  var PackageModule = /*#__PURE__*/Object.freeze({
    ...
    registerLocaleDataLoader: webcomponents.C$2,
    ...
    scroll: n$2,
    setAnimationMode: webcomponents.m$3,
    setCustomElementsScopingRules: webcomponents.p$1,
    setCustomElementsScopingSuffix: webcomponents.l$4,
    setDefaultIconCollection: webcomponents.e,
    setFetchDefaultLanguage: webcomponents.m$4,
